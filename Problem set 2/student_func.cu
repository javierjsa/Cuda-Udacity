                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  //checkCudaErrors(cudaMemcpy(imageOutputRGBA.ptr<unsigned char>(0), d_outputImageRGBA__, sizeof(uchar4) * numPixels,       
  // cudaMemcpyDeviceToHost));
  //copy input array to the GPU (**d_rgbaImage)
  //checkCudaErrors(cudaMemcpy(*d_rgbaImage, *inputImage, sizeof(uchar4) * numPixels, cudaMemcpyHostToDevice));
  //float **d_filter_pointer = &d_filter;
  checkCudaErrors(cudaMemcpy(d_filter, h_filter, sizeof(float) * filterWidth * filterWidth, cudaMemcpyHostToDevice));
  std::cout<<"CudaMemcpy y cudaMalloc filter parece que funciona\n";
}
